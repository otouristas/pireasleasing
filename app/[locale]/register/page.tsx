  ...
  return (
    <main className="min-h-screen bg-white py-20">
      <div className="container max-w-md">
        <h1 className="text-4xl font-bold text-[#0B1B33] mb-8">{locale === 'el' ? 'Εγγραφή' : 'Register'}</h1>
        <form onSubmit={onSubmit} className="space-y-4 bg-[#ECEFF1] p-8 rounded-lg">
          ...
          <button className="btn-primary w-full py-3" disabled={loading}>
            {loading ? (locale === 'el' ? 'Δημιουργία...' : 'Creating...') : (locale === 'el' ? 'Δημιουργία Λογαριασμού' : 'Create account')}
          </button>
        </form>
        <p className="mt-4 text-center text-gray-600">
          {locale === 'el' ? 'Έχετε ήδη λογαριασμό; ' : 'Already have an account? '}
          <Link href={`/${locale}/login`} className="text-[#F9C80E] font-semibold hover:underline">
